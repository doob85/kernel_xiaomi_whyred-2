				...
				unsigned long attrs, bool is_coherent)
{
	...
	if (PageHighMem(page)) {
		...
		while (size > 0) {
			...
			if ((attrs & DMA_ATTR_SKIP_ZEROING) == 0)
					...
					gfp_t gfp, bool coherent)
{
	...
	__dma_clear_buffer(page, size, 0, coherent);
...
static int __init atomic_pool_init(void)
{
	...
	if (dev_get_cma_area(NULL))
		ptr = __alloc_from_contiguous(0, atomic_pool_size, prot,
					      &page, atomic_pool_init, 0);
	else
				     ...
				     unsigned long attrs)
{
	...
	struct page *page;
	void *ptr = NULL;
	bool want_vaddr = ((attrs & DMA_ATTR_NO_KERNEL_MAPPING) == 0)
...
static inline pgprot_t __get_dma_pgprot(unsigned long attrs, pgprot_t prot,
					bool coherent)
{
	if (atrrs & DMA_ATTR_STRONGLY_ORDERED) == 0
